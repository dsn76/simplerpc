...
- **Многоадресная рассылка:** Один вызов может быть исполнен всеми зарегистрированными процессами (`RPC_SEND_ALL`) или только первым (`RPC_SEND_FIRST`); результаты собираются через `libsrpc_lastreq_num()` / `libsrpc_lastreq_get()`.
...
│   ├── libsrpc_mpmcq.c         # Lock-free MPMC-очередь запросов
...
Формат: `XF(политика рассылки, тип возврата, имя, типы параметров...)`. Функции с переменным числом параметров не допускаются.
