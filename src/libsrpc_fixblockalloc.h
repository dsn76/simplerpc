...
typedef struct srpc_pool_fba_s {	// Структура быстрого менеджера блоков фиксированного размера.
		...
		_Atomic(uint32_t)	alloc, free;	// Индексы alloc, free.
...
#define FBA_FOREACH_BLOCK(pool, ptr) size_t i; for(i = 0, ptr = (char*)pool + pool->offset; i < pool->blk_nums; i++, ptr = (char*)pool + pool->offset + ((uintptr_t)i * (uintptr_t)pool->blk_size))
